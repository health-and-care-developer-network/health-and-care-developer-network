...
DECLARE
    ...
    repository_identifier CONSTANT UUID NOT NULL := repository_identifier();

    stuff_identifier CONSTANT UUID NOT NULL := stuff_identifier();
    ...
    stuff_event_identifier CONSTANT UUID NOT NULL := uuid_generate_v4();
    ...
    message CONSTANT VARCHAR NOT NULL := '"' || patient_identifier_nhs_number || '","' || CAST(provider_identifier AS VARCHAR) || '","' || CAST(repository_identifier AS VARCHAR) || CAST(stuff_identifier AS VARCHAR) || '","' || CAST(stuff_event_identifier AS VARCHAR) || '","' || CAST(epoch_in_milliseconds AS VARCHAR) || '","' || repository_event || '"';
